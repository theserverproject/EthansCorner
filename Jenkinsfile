pipeline {
  ...
  stages {
    ...
    stage('Master-Deploy') {
      ...
      steps {
        sh 'sshpass -p $ETHANSCORNERPASSWORD scp -r -oStrictHostKeyChecking=no $WORKSPACE/EthansCorner/SortingAlgorithm/html/ ethanscorner@$SERVER:$ETHANSCORNERLOCATION'
      }
    }
  }
}
